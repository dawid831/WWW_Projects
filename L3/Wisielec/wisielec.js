const words = ["algorytm", "komputer", "program",
    "javascript", "uczelnia", "żółw"];
let selectedWord = "";
let guessedLetters = [];
let wrongGuesses = [];
const maxWrong = 10;
...
function handleKeyClick(e) {
    ...
    if (!btn.classList.contains("used") &&
            ...
            selectedWord.split("").some(l => !guessedLetters.includes(l)))
    {
...
function loadState() {
    ...
    if (state) {
        ...
        if (guessedLetters.includes(key.textContent) ||
                wrongGuesses.includes(key.textContent))
        {
